...
int main() {
    ...
    SSSP_Dijkstra_1::source = x;
    for (int i = 0; i < m; ++i) {
        IO::read(u, v, w);
        SSSP_Dijkstra_1::add_edge(u, v, w);
    }
    SSSP_Dijkstra_1::dijkstra(SSSP_Dijkstra_1::adj);
    vector<int> dist(SSSP_Dijkstra_1::dis.begin() + 1, SSSP_Dijkstra_1::dis.begin() + n + 1);
    SSSP_Dijkstra_1::reset();
    SSSP_Dijkstra_1::dijkstra(SSSP_Dijkstra_1::adj_rev);
    for (int i = 1; i <= n; ++i) {
        dist.at(i - 1) += SSSP_Dijkstra_1::dis.at(i);
    }
    IO::writeln(*max_element(dist.begin(), dist.end()));
