...
def Cmp(n):
    return False if n == 1 else sum(map(lambda x:sum(map(int, str(x))), PrimeFactorsListGenerator(n))) == sum(map(int, str(n)))

print(1) if Cmp(int(input())) else print(0)
