...
int getMoneySpent(vector<int> keyboards, vector<int> drives, int b) {
     ...
     */
    unordered_set<int> setKeyboards (keyboards.begin(), keyboards.end());
    unordered_set<int> setDrives (drives.begin(), drives.end());
    int result = -1;
    int current;
    for (const auto &it0 : setKeyboards) {
        for (const auto &it1 : setDrives) {
            current = it0 + it1;
