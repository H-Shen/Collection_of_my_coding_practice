...
int main() {
  ...
  if (partition0_len == 1) {
    ...
    printf("%s %d %s\n", partition0[0].currentAddress.c_str(), partition0[0].val, partition0[0].nextAddress.c_str());
  }
  else {
      for (i = 0;  i < partition0_len - 1; ++i) {
        partition0[i].nextAddress = partition0[i + 1].currentAddress;
        printf("%s %d %s\n", partition0[i].currentAddress.c_str(), partition0[i].val, partition0[i].nextAddress.c_str());
      }
      partition0[partition0_len - 1].nextAddress = "-1";
      printf("%s %d %s\n", partition0[partition0_len - 1].currentAddress.c_str(), partition0[partition0_len - 1].val, partition0[partition0_len - 1].nextAddress.c_str());
  }
  ...
  if (partition1_len == 1) {
    ...
    printf("%s %d %s\n", partition1[0].currentAddress.c_str(), partition1[0].val, partition1[0].nextAddress.c_str());
  }
  else {
      for (i = 0;  i < partition1_len - 1; ++i)    {
        partition1[i].nextAddress = partition1[i + 1].currentAddress;
        printf("%s %d %s\n", partition1[i].currentAddress.c_str(), partition1[i].val, partition1[i].nextAddress.c_str());
      }

      partition1[partition1_len - 1].nextAddress = "-1";
      printf("%s %d %s\n", partition1[partition1_len - 1].currentAddress.c_str(), partition1[partition1_len - 1].val, partition1[partition1_len - 1].nextAddress.c_str());
  }
  return 0;
}